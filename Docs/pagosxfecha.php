...
class PDF extends TCPDF {
    ...
    $reporteXfecha = (array)$modulo->pagosPorFecha($_POST['fechainicio'], $_POST['fechafin']);
    $campocantidad = 'cantidad';
    $campocargo = 'costorecargo';
    $sumaimporte = array_sum(array_column($reporteXfecha,$campocantidad));
    $sumacargo = array_sum(array_column($reporteXfecha,$campocargo));
    ...
    $pdf->Cell(0, 3, 'REPORTE | INGRESO DESDE ' . $_POST['fechainicio'] . ' HASTA ' . $_POST['fechafin'], 0, 1, 'C');
    $pdf->Ln(10);
    ...
    $i;
    for($i = 0; $i < count($reporteXfecha); $i++){
        ($i%2 == 0) ? $pdf->SetFillColor(161, 208, 246) : $pdf->SetFillColor(255, 255, 255) ;
        $pdf->Ln(6);
        $pdf->SetFont('helvetica', 'B', 8);
        $pdf->SetTextColor(0, 0, 0);
        $cargo = $reporteXfecha[$i]['costo']*100;
        if($reporteXfecha[$i]['descripcion'] == NULL){
            $concepto = "Cuota mensual";
        }else{
            $concepto = "Cuora mensual y recargos " . $cargo . "%";
        }
        $pdf->Cell(23, 5, $reporteXfecha[$i]['fechapago'], 0, 0, 'C', 1);
        $pdf->Cell(27, 5, $reporteXfecha[$i]['cvepago'], 0, 0, 'C', 1);
        $pdf->Cell(22, 5, $reporteXfecha[$i]['cverecibo'], 0, 0, 'L', 1);
        $pdf->Cell(25, 5, $reporteXfecha[$i]['cvecons'], 0, 0, 'C', 1);
        $pdf->Cell(44, 5, $concepto, 0, 0, 'C', 1);
        $pdf->Cell(20, 5, 'MX$ ' . number_format($reporteXfecha[$i]['costorecargo'], 2, ".", ","), 0, 0, 'C', 1);
        $pdf->Cell(30, 5, 'MX$ ' . number_format($reporteXfecha[$i]['cantidad'], 2, ".", ","), 0, 0, 'C', 1);
        $pdf->Cell(77, 5, $reporteXfecha[$i]['comentarios'], 0, 0, 'L', 1);
        if($i == 18){
            $pdf->AddPage();
            $pdf->Ln(15);
            $pdf->SetFont('helvetica', 'B', 9);
    ...
    ob_end_clean();
    $pdf->Output('REPORTE_' . $_POST['fechainicio'] . '_AL_' . $_POST['fechainicio'] . '.pdf', 'I');

...
class PDF extends TCPDF {
    ...
    $niveles = (array)$modulo->verNiveles();
    $consultorios = (array)$modulo->verReporteGeneral();
    $cuota = (array)$modulo->obtenerCuota();
    $totales = (array)$modulo->obtenerTotales();
    ...
    $pdf->Cell(0, 3, 'REPORTE GENERAL | CUOTAS MENSUALES', 0, 1, 'C');
    for($i = 0; $i < count($niveles); $i++){
        ...
        $pdf->Cell(40, 5, strtoupper($niveles[$i]['descripcion']), 0, 0, 'L', 1);
        $pdf->Ln(5);
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->SetFillColor(82, 138, 226);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->Cell(78, 5, 'Propietario', 1, 0, 'C', 1);
        $pdf->Cell(34, 5, 'm2', 1, 0, 'C', 1);
        $pdf->Cell(34, 5, 'Costo x m', 1, 0, 'C', 1);
        $pdf->Cell(34, 5, 'Total', 1, 0, 'C', 1);
            for($j = 0; $j < count($consultorios); $j++){
                if($consultorios[$j]['idplanta'] == $niveles[$i]['idplanta']){
                    $pdf->Ln(5);
                    $pdf->SetFont('helvetica', 'B', 8);
                    $pdf->SetFillColor(255, 255, 255);
                    $pdf->SetTextColor(0, 0, 0);
                    $pdf->Cell(78, 5, $consultorios[$j]['titulo'] . " " . $consultorios[$j]['nombre'] . " " . $consultorios[$j]['apellidop'] . " " . $consultorios[$j]['apellidom'], 1, 0, 'L', 1);
                    $pdf->Cell(34, 5, number_format($consultorios[$j]['dimensiones'], 2, ".", ","), 1, 0, 'C', 1);
                    $pdf->Cell(34, 5, 'MX$ ' . number_format($cuota[0]['costo'], 2, ".", ","), 1, 0, 'C', 1);
                    $pdf->Cell(34, 5, 'MX$ ' . number_format($consultorios[$j]['dimensiones'] * $cuota[0]['costo'], 2, ".", ","), 1, 0, 'C', 1);
                    continue;
                }                
            }
        $pdf->Ln(5);
        ...
        $pdf->SetFillColor(82, 138, 226);        
        $pdf->SetTextColor(0, 0, 0);
        $pdf->Cell(78, 5, 'Total', 1, 0, 'R', 1);
        $pdf->SetFillColor(255, 255, 255);
        $pdf->Cell(34, 5, number_format($totales[$i]['totaldimension'], 2, ".", ","), 1, 0, 'C', 1);
        $pdf->Cell(34, 5, 'MX$ ' . number_format($totales[$i]['totalcosto'], 2, ".", ","), 1, 0, 'C', 1);
        $pdf->Cell(34, 5, 'MX$ ' . number_format($totales[$i]['totalmensualidad'], 2, ".", ","), 1, 0, 'C', 1);
    }
    $pdf->Ln(30);
    ...
    ob_end_clean();
    $pdf->Output('REPORTE' . $cverecibo . '.pdf', 'I');